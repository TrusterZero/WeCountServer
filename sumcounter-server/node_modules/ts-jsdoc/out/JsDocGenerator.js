...
function generate(basePath, config, moduleName, main, options) {
    ...
    const generator = new JsDocGenerator(program, path.relative(basePath, compilerOutDir), moduleName, main, program.getCommonSourceDirectory(), options, compilerOptions.baseUrl);
...
class JsDocGenerator {
    constructor(program, relativeOutDir, moduleName, mainFile, commonSourceDirectory, options, baseUrl) {
        ...
        this.resolvedBaseUrl = baseUrl == null ? null : path.resolve(baseUrl);
    }
    sourceFileToModuleId(sourceFile) {
        if (sourceFile.isDeclarationFile) {
            ...
            else if (this.resolvedBaseUrl != null && fileNameWithoutExt.startsWith(this.resolvedBaseUrl)) {
                fileNameWithoutExt = fileNameWithoutExt.substring(this.resolvedBaseUrl.length + 1);
            }
    ...
    describeProperty(node, isParentClass) {
        ...
        if (initializer != null) {
            if (initializer.expression != null || initializer.kind === ts.SyntaxKind.NoSubstitutionTemplateLiteral) {
