...
async function generateAndWrite(basePath, config, tsConfig) {
    ...
    try {
        packageData = await fs_extra_p_1.readJson(path.join(basePath, "package.json"));
    }
    catch (e) {
    }
    ...
    await fs_extra_p_1.emptyDir(out);
    ...
    const existingClassExampleDirs = exampleDir == null ? null : new Set((await fs_extra_p_1.readdir(exampleDir)).filter(it => it[0] != "." && !it.includes(".")));
    for (const [moduleId, psi] of generator.moduleNameToResult.entries()) {
        ...
        for (const d of copyAndSort(psi.classes)) {
            ...
            if (existingClassExampleDirs != null && existingClassExampleDirs.has(d.name)) {
                ...
                examples = await bluebird_lst_1.default.map((await fs_extra_p_1.readdir(dir)).filter(it => it[0] != "." && it.includes(".")), async (it) => {
                    const ext = path.extname(it);
                    return {
                        name: path.basename(it, ext),
                        content: await fs_extra_p_1.readFile(path.join(dir, it), "utf8"),
                        lang: ext
                    };
                });
            }
        ...
        await fs_extra_p_1.writeFile(path.join(out, moduleId.replace(/\//g, "-") + ".js"), `${externalJsDoc}/** 
 * @module ${moduleId}
 */
